...
for t in os.listdir('./videos/'):
    ...
    for v in os.listdir(video_dir):
        ...
        if not os.path.exists(video_filename + 'vmaf-phone'):
            os.makedirs(video_filename + 'vmaf-phone')
        ...
        for _p in range(p_len):
            ...
            for df in DASH_FILE:
                ...
                os.system(
                    'vmafossexec yuv420p 1920 1080 baseline.yuv out.yuv model/vmaf_v0.6.1.pkl --phone-model --log out.json --thread 0 >> out.log')
                # find vmaf
                f = open('out.log', 'r')
                for l in f:
                    sp = l.split(' = ')
                    if len(sp) == 2:
                        score = float(sp[-1])
                f.close()
                f = open(video_filename + 'vmaf-phone/' + df, 'a')
                f.write(str(score))
                f.write('\n')
                f.close()
